...
class DataReceiverThread(QThread):
    # Change the signal definition to include timestamps
    ...
    def runLoop(self):
        ...
        pc_timestamp = time.time() # Capture this!
        self.dataReady.emit(im.reshape((height, width)), timestamp, pc_timestamp)
...
# UI Text Setup
textItem = pg.TextItem(anchor=(0, 0), color='w', fill=(0, 0, 0, 150))
textItem.setPos(10, 10)  # Top left corner
view.addItem(textItem)

# # Optional: Add a crosshair or marker at the peak pressure point
# peak_marker = pg.ScatterPlotItem(size=10, pen=pg.mkPen('w'), brush=pg.mkBrush('r'))
# view.addItem(peak_marker)
...
# CSV
#----------------------------
# Initialize CSV File (Run once at startup)
csv_filename = "sensor_log.csv"
if not os.path.exists(csv_filename):
    with open(csv_filename, 'w', newline='') as f:
        writer = csv.writer(f)
        writer.writerow(["Tile_Timestamp", "PC_Timestamp", "Sensor_X", "Sensor_Y", "Value"])


def updateImage(image, tile_ts, pc_ts):
    # 1. Visual Heatmap Update
    img.setImage(image.T, levels=[0, 14000])

    # 2. Find Activated Sensors (Value > Threshold)
    threshold = 50 
    activated_coords = np.argwhere(image > threshold) # Returns [[y, x], [y, x]...]
    
    # 3. Log to CSV
    # Warning: If many sensors are active, logging every frame to CSV can be slow.
    # We open the file in 'append' mode.
    with open(csv_filename, 'a', newline='') as f:
        writer = csv.writer(f)
        for y, x in activated_coords:
            val = image[y, x]
            writer.writerow([
                f"{tile_ts:.4f}", # Tile time with 4 decimal places
                f"{pc_ts:.4f}",
                x, y,             # Coordinates
                val               # Sensor pressure value
            ])

    # 4. Update UI Text
    stats_text = (
        f"Tile Time: {datetime.datetime.fromtimestamp(tile_ts).strftime('%H:%M:%S.%f')[:-3]}\n"
        f"Activated Sensors: {len(activated_coords)}\n"
        f"Peak Value: {np.max(image)}"
    )
    textItem.setText(stats_text)
